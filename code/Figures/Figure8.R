...
cpc<- read.csv("epd_s6_8.csv")
cpc_avg<- (data.frame(apply(cpc[,1:5],1,mean)))
colnames(cpc_avg)<- "EPD"
...
access_hist<- read.csv("epd_s6_8.csv")
access_hist_avg<- (data.frame(apply(access_hist[,1:5],1,mean)))
colnames(access_hist_avg)<- "EPD"
access_hist_avg$Timeline <- rep("Historical",36)
access_hist_avg$year<- seq(1979,2014,1)
access_hist_avg$Diff<- rep(0,36)
intercept_access<- rep(mean(access_hist_avg$EPD),2)
setwd("./access/ssp370")
access_ssp370<- read.csv("epd_s6_8.csv")
access_ssp370_avg<- (data.frame(apply(access_ssp370[,1:5],1,mean)))
colnames(access_ssp370_avg)<- "EPD"
hist_access<- data.frame(mean(access_hist_avg$EPD))
colnames(hist_access)<- "EPD"
...
access_ssp585_avg$Diff<- (access_ssp585_avg$EPD -rep(hist_access$EPD,87))
...
miroc_ssp585_avg$Diff<- (miroc_ssp585_avg$EPD -rep(hist_miroc$EPD,87))
...
epd_all<- rbind(cpc_all,access_full,ec_full,miroc_full,mpi_full,mri_full)
...
slope_all$EPD<-c(3,18,3,18,3,18,3,18,3,18)
...
pvalue$EPD<-c(1,23,1,23,1,23,1,23,1,23)
...
library(ggplot2)
ggplot(data = epd_all, aes(x = year, y = EPD)) +
  ...
  geom_text(
    ...
    aes(
      ...
      y = EPD,  # Adjusted Y-position to avoid overlap
...
ggsave("epd_detrend_slope_p_updated.jpeg",width = 12, height=8)
